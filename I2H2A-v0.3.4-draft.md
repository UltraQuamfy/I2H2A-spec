...
#### 3.3.0 Issuer and JSON-LD context

**`issuer`:** Per **[VC-DATA-MODEL-2.0]**, **`issuer`** MAY be a URI string (as in the examples in this specification) or an object with an **`id`** property holding the issuer URI; both forms are conformant.

**`@context`:** The I2H2A extension entry **MUST** be **`https://i2h2a.org/contexts/v1.jsonld`**, matching the document at [`contexts/v1.jsonld`](./contexts/v1.jsonld) in this repository. Deployments **SHOULD** serve that JSON-LD at this HTTPS IRI so JSON-LD processors can resolve it.
...
**`statusSize`** (OPTIONAL)  
Per **[StatusList]**, **`statusSize`** on **`BitstringStatusListEntry`** is the number of status bits consumed at this index. When omitted, it defaults to **1** (single-bit revocation). Omit **`statusSize`** when that default applies; supply it when using multi-status encodings that require more than one bit per index.
...
```json
{
  ...
  "credentialStatus": {
    "id": "https://status.example.com/global-list#67890",
    "type": "BitstringStatusListEntry",
    "statusPurpose": "revocation",
    "statusListIndex": "67890",
    "statusListCredential": "https://status.example.com/global-list"
  }
}
```

*Illustrative:* **`statusListCredential`** is shown as an HTTPS URL that resolves to a **`BitstringStatusListCredential`**. A DID or other URI scheme is permitted by **[StatusList]** only if verifiers in that deployment can dereference it to the status list credential (e.g. via DID URL binding to an HTTPS endpoint).
...
VC **data model** documents and **SD-JWT VC** wire forms are different media types and processing paths (**[VC-DATA-MODEL-2.0]** §6.3, Type-Specific Credential Processing; e.g. `application/vc` vs `application/vc+sd-jwt`). I2H2A secured presentations use the SD-JWT VC profile; the JSON VP shape above is illustrative of logical content, not an alternate on-the-wire encoding.
...
- VC conforms to the field rules defined in §3–§4
...
`encodedList` contains the GZIP-compressed, Base64-encoded bitstring.
...
This section is retained as a placeholder. H2A2A is **not part of v0.3.x** and will not be designed or built until a concrete use case justifies it.
...
eyJhbGciOiJFUzI1NiIsInR5cCI6InZjK3NkLWp3dCIsImtpZCI6ImRpZDpleGFtcGxlOmlzc3VlcjEyMyNrZXktMSJ9..signature-here
...
*Informative:* JSON-shaped credentials in **§3**–**§4** use **`validFrom`** / **`validUntil`** on the VC data model per **[VC-DATA-MODEL-2.0]**. At the JWT layer, SD-JWT VC uses registered claims **`iat`** and **`exp`** (issuance and expiration times); these correspond to **`validFrom`** / **`validUntil`** on the credential when those VC properties are present or derived for display. The compact JWT material above is illustrative; decoded SD-JWT VC payloads MUST be interpreted together with **[RFC9901]** and SHOULD surface validity intervals consistently with those JSON examples.
